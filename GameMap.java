        ...
        for (int i = 0; i < eventList.size(); i++) {
            ...
            if (event.isLive()){
                ...
                if (eventList.get(i).getType() == 0){
                    //判断角色矩形与事件矩形是否交叉
//                    System.out.println(event.isLive()+"----"+gf.actor.getActorRect().intersects(event.getRectangle()));
...
//        overlays.drawCover(id, x, y, g);
    ...
    public ArrayList getBfsResult() {
        return new ArrayList();
    }

    public int[][] copyMap(){
        int[][] mapCopy = new int[row][colum];
        for (int i = 0; i < row; i++) {
    ...
    public boolean isRoom(Site v) {
        int i = v.i();
        int j = v.j();
        if (i < 0 || j < 0 || i >= row || j >= colum) return false;
        return this.airWall[i][j]!=1&&this.airWall[i][j]!=2;
    }
    ...
    public java.util.List<Site> fetchAdjacentSites(Site site) {
        ...
        for (int[] direction  : directions) {
            int newX = site.i() + direction[0];
            int newY = site.j() + direction[1];
            if ((newX != 0 || newY != 0) &&
                    newX >= 0 && newX < row &&
                     newY >= 0 && newY < colum) {
    ...
    public int[][] bfs(int x, int y) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < queueSize; i++) {
                ...
                for (Site neighbor : fetchAdjacentSites(current)) {
                    if (this.isLegalMove(current, neighbor) && mapCopy[neighbor.i()][neighbor.j()] == -1||depth<mapCopy[neighbor.i()][neighbor.j()]) {
                        mapCopy[neighbor.i()][neighbor.j()] = depth;
                        queue.add(neighbor);
                    }
                }
            }
    ...
    public boolean isLegalMove(Site current, Site neighbor) {
        //还需要判断怪物是否可以走到
        if (isWall(neighbor)) {
            return false;
        }
    ...
    public java.util.List<Site> fetchAdjacentSitesAll(Site site) {
        List<Site> neighbors = new ArrayList<>();
        int[][] directions = {
                {-1, 0},  // Up
                {1, 0},   // Down
